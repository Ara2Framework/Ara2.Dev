...
namespace Tecnomips.Ara2_Dev_VS
{
    ...
    //     - An empty json file in the same directory as NewFileItems.vsdir. In
    //       our case we chose Form.AraDesign. Note this file name appears in Editor.vsdir
    ...
    //          - "AraDesign" - This is the base file name. New files will initially
    //             be named as AraDesign1.json, AraDesign2.json... etc.
    ///////////////////////////////////////////////////////////////////////////////
    // Editor.vsdir contents:-
    //    Form.AraDesign|{3085E1D6-A938-478e-BE49-3546C09A1AB1}|#106|80|#109|0|401|0|#107
    //
    // The fields in order are as follows:-
    //    - Form.AraDesign - our empty json file
                                ...
                                
    {
        ...
        private const int MyFormat = 0;
        private const string MyExtension = ".AraDesign";
        
        #region Fields
        private Ara2_Dev_VSPackage myPackage;
        ...
        public EditorPane(Ara2_Dev_VSPackage package)
            : base(null)
        {
            PrivateInit(package);
        }
        ...
        /// <summary>
        /// Initialization routine for the Editor. Loads the list of properties for the teste document 
        /// which will show up in the properties window 
        /// </summary>
        /// <param name="package"></param>
        private void PrivateInit(Ara2_Dev_VSPackage package)
        {
            CanvasProperties = new CanvasProperties(this);
            myPackage = package;
            loading = false;
            ...
            listObjects.Add(CanvasProperties);
            ...
            selContainer.SelectedObjectsChanged += SelectedObjectsChanged;
            ...
            this.editorControl = new MyEditor(this);

            resources.ApplyResources(this.editorControl, "MyEditor", CultureInfo.CurrentUICulture);
            this.editorControl.ChangePendingToSave += OnChange;
        ...
        {
            var vObjS = ((object[])((Microsoft.VisualStudio.Shell.SelectionContainer)sender).SelectedObjects).FirstOrDefault();

            string vName = null;
            try
            {
                vName = ((dynamic)vObjS).Name;
            }
            catch { }

            if (vName!=null && vName!="Canvas")
                this.editorControl.SelectEditObject(new string[] { vName });
        }
        ...
        {
            if (!InstancesTools.ContainsKey(this))
                InstancesTools.Add(this, new List<Type>());

            IVsToolbox toolbox = (IVsToolbox)GetService(typeof(SVsToolbox));

            
            List<Type> ListaJaAdd;
            InstancesTools.TryGetValue(this, out ListaJaAdd);

            #region Carrega ListaJaAdd
                ...
                i = items.Next(1, rgelt, out num))
            {
                ...
                {
                    var vType2 = (Type)voleData.GetData(typeof(Ara2.Dev.IAraDev));
                    if (editorControl.ProjectReferences.Components.Where(a => a == vType2).Any())
                    {
                        if (!ListaJaAdd.Contains(vType2))
                            ListaJaAdd.Add(vType2);
                    }
                }
            }
            #endregion
        ...
        static Dictionary<EditorPane, List<Type>> InstancesTools = new Dictionary<EditorPane, List<Type>>();
        public new void Dispose()
        {
            List<Type> vComponentes;
            List<Type> vComponetesRemover = new List<Type>();
            if (InstancesTools.TryGetValue(this, out vComponentes))
            {
                foreach (var vC in vComponentes)
                {
                    // Se não achar mais utilidade para este componente ele tira
                    if (!InstancesTools.Where(a => a.Key != this && a.Value.Where(b => b == vC).Any()).Any())
                    {
                        vComponetesRemover.Add(vC);
                    }
                }
            }
            ...
            lock (InstancesTools)
            {
                RemoverItensInuteisToolBox(vComponetesRemover);
                vComponetesRemover.Clear();
                vComponetesRemover = null;
                InstancesTools.Remove(this);
            }
        ...
        {
            //(new System.Threading.Thread(ReloadObjectList)).Start();
            ReloadObjectList();
        }

        public IEnumerable<DynamicProxy> Objects;
        public bool isReloadObjectList = false;
        private void ReloadObjectList()
        {
            var vObjBytes = editorControl.ServiceHost.Cliente.Channel<byte[]>(a=>a.GetOjectsDevAll());

            try
            {
                AppDomain.CurrentDomain.AssemblyResolve += CurrentDomain_AssemblyResolve;
                BinaryFormatter bf = new BinaryFormatter();
                var vObj = bf.Deserialize(new MemoryStream(vObjBytes));
                var vObj2 = ((object[])vObj).Select(a => new DynamicProxy(a, this));
                Objects = vObj2.ToList();
                selContainer.SelectableObjects = Objects.Select(a => (object)a).Union(new object[] { CanvasProperties }).ToArray();
                UpdateSelection();

                isReloadObjectList = true;
            }
            catch (Exception err)
            {
                throw new Exception("Erro ao receber objetos da interface web.",err);
            }
            finally
            {
                AppDomain.CurrentDomain.AssemblyResolve -= CurrentDomain_AssemblyResolve;
        ...
        #region ReturnSetValue
        public class CReturnSetValue
        {
            public static object vLockNewID = new object();
            public static int NewID = 1;

            public CReturnSetValue()
            {
                lock (vLockNewID)
                {
                    ID = NewID;
            ...
            public int ID;
            public bool FinallySetValueEnd = false;
            public Exception FinallySetValueError = null;
        }

        static List<CReturnSetValue> ReturnSetValue = new List<CReturnSetValue>();

        public static void WaitReturnSetValueAll()
        {
            while(ReturnSetValue.Where(a=>a.FinallySetValueEnd==false).Any())
                System.Windows.Forms.Application.DoEvents();
        }

        public static CReturnSetValue GetNewReturnSetValue()
        {
            var vTmp = new CReturnSetValue();
            ReturnSetValue.Add(vTmp);
            return vTmp;
        }

        public void FinallySetValue(int ID, Exception err)
        {
            lock (ReturnSetValue)
            {
                var vReturn = ReturnSetValue.Find(a => a.ID == ID);

                vReturn.FinallySetValueEnd = true;
                vReturn.FinallySetValueError = err;
            }
        }

        public static void FinallySetValueClear(int ID)
        {
            lock (ReturnSetValue)
            {
                ReturnSetValue.RemoveAll(a => a.ID == ID);
            }
        }
        ...
        Assembly CurrentDomain_AssemblyResolve(object sender, ResolveEventArgs args)
        {
            ...
            try
            {
                ...
                vTmp = editorControl.ProjectReferences.Assemblys.Where(a => a.GetName().Name == (new AssemblyName(args.Name)).Name).FirstOrDefault();
                if (vTmp != null)
                    return vTmp;
                else
        ...
        sealed class PreMergeToMergedDeserializationBinder : SerializationBinder
        {
            ...
            public static System.Threading.ThreadLocal<IEnumerable<Type>> TypesThread = new System.Threading.ThreadLocal<IEnumerable<Type>>();
            public PreMergeToMergedDeserializationBinder(IEnumerable<Type> vTypes)
            {
                Types = vTypes;
            }
            ...
            {
                var vTmpA = AppDomain.CurrentDomain.GetAssemblies();

                TypesThread.Value = Types;

                Type vTmpType = Type.GetType(typeName +  ", " + assemblyName, OnAssemblyResolve, OnTypeResolver);
                
                TypesThread.Value = null;

                if (vTmpType != null)
                    return vTmpType;
                else
                    throw new Exception(typeName + " , " +  assemblyName + " not found");
                //Type typeToDeserialize2 = Type.GetType(typeName);
                //if (typeToDeserialize2 != null)
                //    return typeToDeserialize2;

                //var vTmp = Types.Where(a => a.FullName == typeName).FirstOrDefault();
                //if (vTmp != null)
                //    return vTmp;
                //else
                //{
                //    // .Where(a => a.FullName.Split(',')[0] == assemblyName.Split(',')[0])
                //    foreach (Assembly a in AppDomain.CurrentDomain.GetAssemblies())
                //    {
                //        try
                //        {
                //            var vName2 = (typeName.IndexOf(",") != -1 ? typeName.Replace(typeName.Split(',')[2], a.FullName.Split(',')[1]) : typeName);

                //            Type typeToDeserialize = a.GetType(vName2);
                //            if (typeToDeserialize != null)
                //                return typeToDeserialize;
                //            else
                //            {
                //                typeToDeserialize = a.GetTypes().Where(b => b.FullName == typeName || b.FullName == vName2).FirstOrDefault();
                //                if (typeToDeserialize != null)
                //                    return typeToDeserialize;
                //            }
                //        }
            ...
            private static Assembly OnAssemblyResolve(AssemblyName vName)
            {
                try
                {
                    Assembly vAS;
                    //vAS  = Assembly.LoadWithPartialName(vName.Name);
                    //if (vAS == null)
                    //{
                    vAS = Assembly.LoadWithPartialName(vName.Name);
                    //vAS = Assembly.Load(vName.Name);
                    if (vAS == null)
                    {
                        vAS = TypesThread.Value.Where(a => a.Assembly.GetName().Name == vName.Name).Select(a => a.Assembly).FirstOrDefault();
                        if (vAS == null)
                            throw new Exception("");
                    }

                    var vTmp = vAS.GetTypes().First();
                    //}
                    return vAS;
                }
                catch (Exception err)
            ...
            public static Type OnTypeResolver(Assembly As, string vTypeName, bool Resu)
            {
                try
                {
                    Debug.Print("OnTypeResolver " + vTypeName + " , " + As.FullName + " " + Resu);
                    Type vType = As.GetType(vTypeName, false, true);
                    if (vType == null)
                        throw new Exception("");
                    return vType;
                } 
        ...
        public void SetObjectsEdit(string[] Names)
        {
            if (Objects != null)
            {
                var vTMp = Objects.Where(a => Names.Contains(((DynamicProxy)a).Name));
                selContainer.SelectedObjects = new ArrayList(vTMp.ToArray());
                UpdateSelection();
            }
        }
        ...
        {
            get { return this.editorControl.PendingToSave; }
        }
        ...
        private void setupCommands()
        {
            ...
            if (null != mcs)
            {
                ...
                //addCommand(mcs, GuidList.guidAra2_Dev_VSCmdSet, (int)PkgCmdIDList.icmdStrike,
                ...
                //addCommand(mcs, VSConstants.GUID_VSStandardCommandSet97, (int)VSConstants.VSStd97CmdID.MultiLevelRedo,
                //                new EventHandler(onTeste), new EventHandler(onQueryTeste));

                //{
                //    var commandEvent = new EventHandler(onTeste);
                //    var queryEvent = new EventHandler(onQueryTeste);
        ...
        #region Teste 
        //private void onQueryTeste(object sender, EventArgs e)
        //{
        //    OleMenuCommand command = (OleMenuCommand)sender;
        //    command.Enabled = true; //editorControl.ServiceHost.Cliente.Channel(a => a.GetQueryCopy());
        //}
        //public void onTeste(object sender = null, EventArgs e = null)
        //{
        //    //System.Windows.Forms.Clipboard.SetData("iAraDev[]", editorControl.ServiceHost.Cliente.Channel(a => a.GetObjectBySendoEditado()));
        ...
        /// <param name="e">  Not used.</param>
        public void onSelectAll(object sender=null, EventArgs e=null)
        {
            editorControl.ServiceHost.Cliente.Channel(a=>a.SelectAll());
        }
        ...
        {
            OleMenuCommand command = (OleMenuCommand)sender;
            command.Enabled = editorControl.ServiceHost.Cliente.Channel(a=>a.GetQueryCopy());
        }
        ...
        /// <param name="sender">  This can be cast to an OleMenuCommand.</param>
        /// <param name="e">  Not used.</param>
        public void onCopy(object sender=null, EventArgs e=null)
        {
            System.Windows.Forms.Clipboard.SetData("iAraDev[]", editorControl.ServiceHost.Cliente.Channel(a=>a.GetObjectBySendoEditado()));
        }
        ...
        {
            OleMenuCommand command = (OleMenuCommand)sender;
            command.Enabled = editorControl.ServiceHost.Cliente.Channel(a=>a.GetQueryCutOrDelete());
        }
        ...
        /// <param name="sender">  This can be cast to an OleMenuCommand.</param>
        /// <param name="e">  Not used.</param>
        public void onCut(object sender = null, EventArgs e = null)
        {
            System.Windows.Forms.Clipboard.SetData("iAraDev[]", editorControl.ServiceHost.Cliente.Channel(a=>a.GetObjectBySendoEditado()));
            editorControl.ServiceHost.Cliente.Channel(a=>a.Delete());
        }

        /// <summary>
        /// Handler for our Delete command.
        /// </summary>
        public void onDelete(object sender = null, EventArgs e = null)
        {
            editorControl.ServiceHost.Cliente.Channel(a => a.Delete());
        }
        ...
        /// <param name="sender">  This can be cast to an OleMenuCommand.</param>
        /// <param name="e">  Not used.</param>
        public void onQueryUndo(object sender = null, EventArgs e = null)
        {
            OleMenuCommand command = (OleMenuCommand)sender;
            try
            {
                command.Enabled = editorControl.ServiceHost.Cliente.Channel(a => a.CatUndo());
            }
            catch(Exception err)
            {
        ...
        public void Invoke_onQueryUndo()
        {
            IMenuCommandService mcs = GetService(typeof(IMenuCommandService)) as IMenuCommandService;
            OleMenuCommand Command = (OleMenuCommand)mcs.FindCommand(new CommandID(VSConstants.GUID_VSStandardCommandSet97, (int)VSConstants.VSStd97CmdID.MultiLevelUndo));
            onQueryUndo(Command, EventArgs.Empty);
        }
        ...
        {
            editorControl.ServiceHost.Cliente.Channel(a => a.Undo());
        }
        ...
        /// <param name="sender">  This can be cast to an OleMenuCommand.</param>
        /// <param name="e">  Not used.</param>
        public void onQueryRedo(object sender=null, EventArgs e=null)
        {
            OleMenuCommand command = (OleMenuCommand)sender;
            try
            {
                command.Enabled = editorControl.ServiceHost.Cliente.Channel(a => a.CatRedo());
            }
            catch { command.Enabled = false; }
        }

        public void Invoke_onQueryRedo()
        {
            IMenuCommandService mcs = GetService(typeof(IMenuCommandService)) as IMenuCommandService;
            OleMenuCommand Command = (OleMenuCommand)mcs.FindCommand(new CommandID(VSConstants.GUID_VSStandardCommandSet97, (int)VSConstants.VSStd97CmdID.MultiLevelRedo));
            onQueryRedo(Command, EventArgs.Empty);
        }
        ...
        {
            editorControl.ServiceHost.Cliente.Channel(a => a.Redo());
        }
        ...
        //    CommandID contextMenuID = new CommandID(GuidList.guidAra2_Dev_VSCmdSet, PkgCmdIDList.IDMX_RTF);
        ...
        {
            pClassID = GuidList.guidAra2_Dev_VSEditorFactory;
            return VSConstants.S_OK;
        }
        ...
        int IPersistFileFormat.Load(string pszFilename, uint grfMode, int fReadOnly)
        {
            ...
            try
            {
                ...
                editorControl.FileProject = (dte.Solution.FindProjectItem(pszFilename)).ContainingProject.FullName;
                editorControl.File = pszFilename;
        ...
        int IVsFileBackup.BackupFile(string pszBackupFileName)
        {
            try
            {
                editorControl.Save(pszBackupFileName);
                backupObsolete = false;
            }
            catch (ArgumentException)
            {
        ...
        public int IsSupported(Microsoft.VisualStudio.OLE.Interop.IDataObject pDO)
        {
            ...
            if (oleData.GetDataPresent(typeof(Ara2.Dev.IAraDev)))
            {

                List<Type> vComponentes = new List<Type>();
                if (InstancesTools.TryGetValue(this, out vComponentes))
                {
                    var vType = (Type)oleData.GetData(typeof(Ara2.Dev.IAraDev));
                    var vAraDevComponent = vType.GetCustomAttribute<Ara2.Dev.AraDevComponent>();
                    if (vComponentes.Contains(vType) && vAraDevComponent!=null && vAraDevComponent.DiplayToolBar)
                        return VSConstants.S_OK;
                    else
        ...
        public int ItemPicked(Microsoft.VisualStudio.OLE.Interop.IDataObject pDO)
        {
            ...
            try
            {
                editorControl.ServiceHost.Cliente.Channel(a => a.AddObject(vType.AssemblyQualifiedName));
                return VSConstants.S_OK;
            }
            catch
            {
        ...
        private void SetReadOnly(bool _isFileReadOnly)
        {
            ...
            if (_isFileReadOnly)
            {
                ...
                {
                    editorCaption = "Arquivo somente leitura";
                }
            }
        ...
        private void OnFileChangeEvent(object sender, System.EventArgs e)
        {
            ...
            try
            {
                message = this.GetResourceString("@101");    //get the message string from the resource
            }
            catch { message = "O Arquivo foi alterado na sua origem. Deseja recarregar e discartar oque foi editado atualmente?"; }
            IVsUIShell VsUiShell = (IVsUIShell)GetService(typeof(SVsUIShell));
            int result = 0;
        ...
        public void FileCsAddReferenceProject(string vFileNew)
        {       
            string vFile = editorControl.File;
            DTE dte = (DTE)GetService(typeof(DTE));

            var vFileObj = (dte.Solution.FindProjectItem(vFile));

            System.IServiceProvider oServiceProvider = new ServiceProvider((Microsoft.VisualStudio.OLE.Interop.IServiceProvider)dte);
            Microsoft.Build.Evaluation.Project oBuildProject = Microsoft.Build.Evaluation.ProjectCollection.GlobalProjectCollection.GetLoadedProjects(vFileObj.ProjectItems.ContainingProject.FullName).Single();
            Microsoft.Build.Evaluation.ProjectProperty oGUID = oBuildProject.AllEvaluatedProperties.Single(oProperty => oProperty.Name == "ProjectGuid");
            ...
            //string szItemPath = (string)vFileObj.Properties.Item("FullPath").Value;
            
            //string szNewItemPath = szItemPath.Replace(Path.GetExtension(szItemPath), ".cs");

            if (!File.Exists(vFileNew))
                File.WriteAllText(vFileNew, " ");

            if (dte.Solution.FindProjectItem(vFileNew) == null)
            {
                uint nItemId;
                ErrorHandler.ThrowOnFailure(oVsHierarchy.ParseCanonicalName(editorControl.File, out nItemId));
                ErrorHandler.ThrowOnFailure(oVsProject.AddItem(nItemId, VSADDITEMOPERATION.VSADDITEMOP_OPENFILE, vFileNew, 1, new string[] { vFileNew }, IntPtr.Zero, oAddResult));

                //var vFileCs = (dte.Solution.FindProjectItem(editorControl.FileCs));
                vFileObj.ProjectItems.AddFromFile(vFileNew);
            }
        }
        #endregion
    }
